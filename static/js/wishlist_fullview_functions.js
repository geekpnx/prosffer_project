...
function updateWishList() {
    const wishListItemsDropdown = document.querySelector('#wishlist-items tbody');
    const totalPriceElementDropdown = document.getElementById('total-price-display');
    const itemCountElement = document.getElementById('wishlist-count');

    const wishListItemsFullView = document.querySelector('#wishlist-items-body'); // Full view target
    const totalPriceElementFullView = document.getElementById('total-price-display-full'); // Total price for full view
    
    wishListItemsDropdown.innerHTML = ''; // Clear existing items in dropdown
    wishListItemsFullView.innerHTML = ''; // Clear existing items in full view
    totalItems = 0; // Reset total items count

    wishList.forEach(item => {
        // Populate Dropdown View
        const rowDropdown = createWishlistRow(item);
        wishListItemsDropdown.appendChild(rowDropdown);
        
        // Populate Full View
        const rowFullView = createWishlistRow(item); // Reuse function to create row
        wishListItemsFullView.appendChild(rowFullView);

        totalItems += item.quantity; // Update total items count
    });

    totalPriceElementDropdown.textContent = `${totalPrice.toFixed(2)} €`; // Update total price in dropdown
    totalPriceElementFullView.textContent = `${totalPrice.toFixed(2)} €`; // Update total price in full view
...
function createWishlistRow(item) {
    ...
    storeCell.textContent = item.storeName;
...
function addToList(productId, storeName, productName, productPrice, productCurrency, productImage) {
    ...
    // Open the wishlist dropdown when the user adds a product
